...
#' This prepare tidy data that can be used for later analysis.
  ...
  analyze <- function(type){
    ...
    onlyMeanAndSd <- function(x){
      ...
      for (i in 1:length(x)){
        ...
        condition <- length(grep("([Mm]ean+)|([Ss]td+)", paste(features$FeatureName[i]), value = TRUE))>0
        if (condition){
